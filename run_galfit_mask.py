...
def run_galfit_no_psf(galaxy_sample,WISE_dir,sample_txt_name_nopsf):
    ...
    for n in range(0,len(galaxy_sample)):
       ...
       try:
          ...
          t = '/mnt/astrophysics/kconger_wisesize/github/'+str(WISE_dir)+'/'+galaxy_sample[n]['prefix']+'-unwise-w3-log.txt'
...
#below this line is a test for running the scripts external to ipython environment!
   ...
   if '-txtfilepsf' in sys.argv:
      p = sys.argv.index('-txtfilepsf')
      txtfilename2 = sys.argv[p+1]
   

   if psf:
      run_galfit_psf(vf,'gal_output',txtfilename,txtfilename2)
